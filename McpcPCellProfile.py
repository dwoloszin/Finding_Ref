...
def McpcPCellProfile(TEC):
  ...
  if os.path.exists(pathToImport):   
    ...
    Frame = TratarSync.processArchive(Frame,['NodeId','McpcPCellProfileId'])
...
def tratarArchive(Frame):
  ...
  try:
    Frame = SplitValues.processArchive2(Frame,'rsrpCandidateA5')
    Frame = SplitValues.processArchive2(Frame,'rsrpCandidateB2')
    Frame = SplitValues.processArchive2(Frame,'rsrpCritical')
    Frame = SplitValues.processArchive2(Frame,'rsrpSearchZone')
  except:
    pass
...
frame_tableList.drop(frame_tableList[frame_tableList['TableList'] != 'McpcPCellProfile'].index, inplace=True)
